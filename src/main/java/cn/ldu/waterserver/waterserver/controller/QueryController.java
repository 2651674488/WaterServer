...
//    }
    ...
    //费用查询
    @GetMapping("/cost")
    public NormalResponse<ExpenseInquiry> queryExpenseInquiry(Integer region_id,String device_code,int page,int page_count){
        List<ExpenseInquiry> entity = expenseInquiryService.queryExpenseInquiry(region_id,device_code,page,page_count);
        NormalResponse<ExpenseInquiry> res = new NormalResponse<>();
        res.setData(entity);
        res.setTotal(entity.size());
        return res;
    }
    //查询流水
    @GetMapping("/flow")
    ...
    //操作日志
    @GetMapping("/opt")
    public NormalResponse<OperationLog> queryOperationLog(String operation_type, int page, int page_count){
        List<OperationLog> entity = operationLogService.queryOperationLog(operation_type,page,page_count);
    ...
    //定时任务
    @GetMapping("/schedule")
    public NormalResponse<ScheduledTasks> queryScheduledTasks(String water_meter_number,int page, int page_count){
        List<ScheduledTasks> entity = scheduledTasksService.queryScheduledTasks(water_meter_number,page,page_count);
